...
/// @notice An example contract using the Sdex V3 flash function
contract PairFlash is ISdexV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `SdexV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        ISdexV3Pool pool = ISdexV3Pool(PoolAddress.computeAddress(factory, poolKey));
